...
//output molecular data
//ideally, we will deal natively in sdf and only use openbabel to convert for alternative formats
void result_info::writeStr(std::stringstream &mystream, bool include_atom_terms, const weighted_terms *wt, int modelnum)
{

	mystream << molstr;
	//now sd data
	mystream << "> <minimizedAffinity>\n";
	mystream << std::fixed << std::setprecision(5) << energy << "\n\n";
	//RMSD
	mystream << "> <rmsdLowerBound>\n";
	mystream << std::fixed << std::setprecision(5) << rmsd_lb << "\n\n";
	mystream << "> <rmsdUpperBound>\n";
	mystream << std::fixed << std::setprecision(5) << rmsd_ub << "\n\n";

	if (rmsd >= 0)
	{
		mystream << "> <minimizedRMSD>\n";
		mystream << std::fixed << std::setprecision(5) << rmsd << "\n\n";
	}
	...
	if (include_atom_terms)
	{
		std::stringstream astr;
		writeAtomValues(astr, wt);
		mystream << "> <atomic_interaction_terms>\n";
		mystream << astr.str() << "\n\n";
	}
