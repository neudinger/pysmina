		...
		unsigned desired_root, const vector<int>& norotate)
{
	...
	//finds the root atom by copying the molecule, deleting each atom in turn, and finding the sizes of the resulting pieces
	{
		...
		{
			smrsi = smrsi > frag_list.at(j).size() ?
														smrsi :
														frag_list.at(j).size();
		}
		...
		if (fecund)
		{
			...
			for (unsigned int i = 0; i < rigid_fragments.size(); i++)
			{
				if (FindBondedPiece((*tree.find(position)).second.atoms,
						rigid_fragments.at(i),
						first_atom, second_atom, first_atom_rank,
						second_atom_rank, mol, atoms_moved))
				{
					...
					tree.insert(
							pair<unsigned int, obbranch>(tree.size(), sprog)); //adds the current branch to the tree
		...
		unsigned int depth)
{
	...
	for (unsigned int i = 0; i < tree.size(); i++)
	{
				...
				it != tree[i].rigid_with.end(); it++)
		{
			...
			{
				new_order.insert(make_pair(atoms[j],
						current_atom_index));
				current_atom_index++;
			}
		}
	...
	for (set<unsigned int>::iterator it =
	...
	for (unsigned int i = 1; i < tree.size(); i++)
	{
				...
				it != tree[i].parents.begin();)
		{
			...
			if (tree[*it].children.size() == 0)
			{
				...
				{
					if (pstack.top().atoms[pos].a.number == pnum)
						break;
				}
